<?php
class URL{
	public static function createLinkPicture($folderName, $fileName, $module = 'backend')
	{
		$linkFile = UPLOAD_PATH . $folderName . DS . $fileName;
		if(file_exists($linkFile))
		{
			return UPLOAD_URL . $folderName . DS . $fileName;
		}
		$pictureDefault = $module == 'backend' ? '60x90-default.jpg' : 'no-img.jpg';
		
		return UPLOAD_URL . $pictureDefault;
	}

	public static function createLinkBookDetail($bookID, $categoryID)
	{
		return self::createLink('frontend', 'book', 'item', ['id' => $bookID, 'category_id' => $categoryID], "categoryName/Detail-$categoryID-$bookID.html");
	}
	...
	{
		header('location: '. self::createLink('frontend', 'index' , 'notice', ['type' => 'not-url'], 'notice-not-url.html'));
		exit();
	}
